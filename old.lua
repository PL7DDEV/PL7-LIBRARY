...
function PL7:CreateWindow(opts)
    ...
    ctrlF.Size=UDim2.new(0,92,0,28); ctrlF.Position=UDim2.new(1,-96,0.5,-14)
    Win._searchBtn=searchBtn
    mkDrag(topbar,root)

    -- barra de pesquisa (oculta inicialmente)
    local searchBar=mkFrm(root,UDim2.new(1,-TAB_W-12,0,32),UDim2.new(0,TAB_W+6,0,57),T.Toggle)
    ...
    local searchResults=mkFrm(root,UDim2.new(1,-TAB_W-12,0,0),UDim2.new(0,TAB_W+6,0,91),T.Sub)
    ...
    -- funcao de busca: varre todos os elementos dos tabs
    local function doSearch(query)
            ...
            for _,child in pairs(tab._content:GetDescendants()) do
                if (child:IsA("TextLabel") or child:IsA("TextButton")) and child.Text~="" then
                    ...
                    if txt:find(query,1,true) and found<8 then
                        ...
                        -- icone de tab
                        local tabIco=mkLbl(rb,tab._iconL and tab._iconL.Text or "·",11,T.Accent,false,Enum.TextXAlignment.Right)
                        ...
                        rb.MouseButton1Click:Connect(function()
                            -- navega para a aba do resultado
                            tab._btn.MouseButton1Click:Fire()
                            -- scroll para o elemento
                            pcall(function()
                                local pos=child:FindFirstAncestorWhichIsA("Frame") and child.AbsolutePosition
                                if pos then tab._content.CanvasPosition=Vector2.new(0,math.max(0,pos.Y-tab._content.AbsolutePosition.Y-20)) end
                            end)
                            searchOpen=false; searchBar.Visible=false; searchResults.Visible=false
                            searchBox.Text=""
                        end)
                    end
                end
        ...
        if found==0 then
            local nf=mkLbl(searchResults,"Nenhum resultado encontrado.",11,T.TextDim)
            nf.Size=UDim2.new(1,0,0,24); nf.ZIndex=31; searchResults.Visible=true
        end
    end
    ...
    UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then
            if searchOpen and not searchBar:IsAncestorOf(inp.Target or Instance.new("Frame")) and inp.Target~=searchBtn then
                if inp.Target and not searchResults:IsAncestorOf(inp.Target) then
    ...
    local function buildSettings()
        ...
        local tDrop_hdr=mkFrm(sp,UDim2.new(1,0,0,38),nil,T.Toggle)
        mkCorner(tDrop_hdr,8); mkStroke(tDrop_hdr,T.Border,1,0.25)
        local tDrop_lbl=mkLbl(tDrop_hdr,"  🎨  "..Win._themeName,13,T.Text)
        tDrop_lbl.Size=UDim2.new(1,-32,1,0)
        local tDrop_arr=mkLbl(tDrop_hdr,"▾",12,T.TextDim,true,Enum.TextXAlignment.Center)
        tDrop_arr.Size=UDim2.new(0,28,1,0); tDrop_arr.Position=UDim2.new(1,-28,0,0)
        local tDrop_box=mkFrm(sp,UDim2.new(1,0,0,0),nil,T.Sub)
        tDrop_box.AutomaticSize=Enum.AutomaticSize.Y; tDrop_box.Visible=false
        mkCorner(tDrop_box,8); mkStroke(tDrop_box,T.Accent,1,0.55)
        mkList(tDrop_box,Enum.FillDirection.Vertical,1); mkPad(tDrop_box,4,4,6,6)
        for _,tn in ipairs(tnames) do
            local th2=PL7.Themes[tn]
            local ob=mkBtn(tDrop_box,tn,12,th2.Accent,Color3.new(0,0,0))
            ob.BackgroundTransparency=1; ob.Size=UDim2.new(1,0,0,32); ob.TextXAlignment=Enum.TextXAlignment.Left; mkPad(ob,0,0,10,10)
            -- cor de destaque na lateral
            ...
            ob.MouseButton1Click:Connect(function()
                Win:ApplyTheme(tn); tDrop_lbl.Text="  🎨  "..tn
        ...
        -- Dropdown de tradução
        secHdr("IDIOMA / TRADUÇÃO")
        local langs={
            {"🇧🇷 Português (BR)",  "pt_BR"},
            {"🇵🇹 Português (PT)",  "pt_PT"},
            {"🇺🇸 English (US)",    "en_US"},
        ...
        -- tabela de strings traduzidas (expansível)
        local Translations={
            pt_BR={open="ABRIR PAINEL",close="Fechar",settings="Config",search="Pesquisar...",theme="Tema",size="Tamanho",opacity="Opacidade",language="Idioma",apply="Aplicar",reset="Resetar"},
            pt_PT={open="ABRIR PAINEL",close="Fechar",settings="Config",search="Pesquisar...",theme="Tema",size="Tamanho",opacity="Opacidade",language="Idioma",apply="Aplicar",reset="Repor"},
            en_US={open="OPEN PANEL",close="Close",settings="Config",search="Search...",theme="Theme",size="Size",opacity="Opacity",language="Language",apply="Apply",reset="Reset"},
            en_GB={open="OPEN PANEL",close="Close",settings="Config",search="Search...",theme="Theme",size="Size",opacity="Opacity",language="Language",apply="Apply",reset="Reset"},
            es={open="ABRIR PANEL",close="Cerrar",settings="Config",search="Buscar...",theme="Tema",size="Tamaño",opacity="Opacidad",language="Idioma",apply="Aplicar",reset="Restablecer"},
            es_MX={open="ABRIR PANEL",close="Cerrar",settings="Config",search="Buscar...",theme="Tema",size="Tamaño",opacity="Opacidad",language="Idioma",apply="Aplicar",reset="Restablecer"},
            fr={open="OUVRIR",close="Fermer",settings="Config",search="Chercher...",theme="Thème",size="Taille",opacity="Opacité",language="Langue",apply="Appliquer",reset="Réinitialiser"},
            de={open="ÖFFNEN",close="Schließen",settings="Config",search="Suchen...",theme="Design",size="Größe",opacity="Deckkraft",language="Sprache",apply="Anwenden",reset="Zurücksetzen"},
            it={open="APRI PANNELLO",close="Chiudi",settings="Config",search="Cerca...",theme="Tema",size="Dimensione",opacity="Opacità",language="Lingua",apply="Applica",reset="Ripristina"},
            ru={open="ОТКРЫТЬ",close="Закрыть",settings="Настройки",search="Поиск...",theme="Тема",size="Размер",opacity="Прозрачность",language="Язык",apply="Применить",reset="Сбросить"},
            zh_CN={open="打开面板",close="关闭",settings="设置",search="搜索...",theme="主题",size="大小",opacity="透明度",language="语言",apply="应用",reset="重置"},
            zh_TW={open="開啟面板",close="關閉",settings="設定",search="搜尋...",theme="主題",size="大小",opacity="透明度",language="語言",apply="套用",reset="重設"},
            ja={open="パネルを開く",close="閉じる",settings="設定",search="検索...",theme="テーマ",size="サイズ",opacity="不透明度",language="言語",apply="適用",reset="リセット"},
            ko={open="패널 열기",close="닫기",settings="설정",search="검색...",theme="테마",size="크기",opacity="불투명도",language="언어",apply="적용",reset="초기화"},
            ar={open="فتح اللوحة",close="إغلاق",settings="إعدادات",search="بحث...",theme="ثيم",size="الحجم",opacity="الشفافية",language="اللغة",apply="تطبيق",reset="إعادة"},
            tr={open="PANELİ AÇ",close="Kapat",settings="Ayarlar",search="Ara...",theme="Tema",size="Boyut",opacity="Opaklık",language="Dil",apply="Uygula",reset="Sıfırla"},
            pl={open="OTWÓRZ PANEL",close="Zamknij",settings="Ustawienia",search="Szukaj...",theme="Motyw",size="Rozmiar",opacity="Przezroczystość",language="Język",apply="Zastosuj",reset="Resetuj"},
            nl={open="PANEEL OPENEN",close="Sluiten",settings="Instellingen",search="Zoeken...",theme="Thema",size="Grootte",opacity="Doorzichtigheid",language="Taal",apply="Toepassen",reset="Resetten"},
            sv={open="ÖPPNA PANEL",close="Stäng",settings="Inställningar",search="Sök...",theme="Tema",size="Storlek",opacity="Genomskinlighet",language="Språk",apply="Tillämpa",reset="Återställ"},
            vi={open="MỞ BẢNG",close="Đóng",settings="Cài đặt",search="Tìm kiếm...",theme="Giao diện",size="Kích thước",opacity="Độ mờ",language="Ngôn ngữ",apply="Áp dụng",reset="Đặt lại"},
            th={open="เปิดแผง",close="ปิด",settings="ตั้งค่า",search="ค้นหา...",theme="ธีม",size="ขนาด",opacity="ความโปร่งแสง",language="ภาษา",apply="ใช้",reset="รีเซ็ต"},
            id={open="BUKA PANEL",close="Tutup",settings="Pengaturan",search="Cari...",theme="Tema",size="Ukuran",opacity="Opacity",language="Bahasa",apply="Terapkan",reset="Setel ulang"},
        }
        Win._lang="pt_BR"

        local lDrop_open=false
        local lDrop_hdr=mkFrm(sp,UDim2.new(1,0,0,38),nil,T.Toggle)
        mkCorner(lDrop_hdr,8); mkStroke(lDrop_hdr,T.Border,1,0.25)
        local lDrop_lbl=mkLbl(lDrop_hdr,"  🌐  Português (BR)",13,T.Text); lDrop_lbl.Size=UDim2.new(1,-32,1,0)
        local lDrop_arr=mkLbl(lDrop_hdr,"▾",12,T.TextDim,true,Enum.TextXAlignment.Center); lDrop_arr.Size=UDim2.new(0,28,1,0); lDrop_arr.Position=UDim2.new(1,-28,0,0)
        local lDrop_box=mkFrm(sp,UDim2.new(1,0,0,0),nil,T.Sub)
        lDrop_box.AutomaticSize=Enum.AutomaticSize.Y; lDrop_box.Visible=false
        mkCorner(lDrop_box,8); mkStroke(lDrop_box,T.Accent,1,0.55)
        mkList(lDrop_box,Enum.FillDirection.Vertical,1); mkPad(lDrop_box,4,4,6,6)
        for _,lang in ipairs(langs) do
            ...
            ob.MouseButton1Click:Connect(function()
                ...
                if tr then
                    floatBtn.Text=tr.open
                    Win._searchBtn.Text="🔍"
                end
                PL7:Notify({Title="Idioma",Text=lang[1].." aplicado!",Type="info",Duration=2})
            end)
        end
